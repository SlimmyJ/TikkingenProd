...
export async function exportPrint(root = document) {
  ...
  canvases.forEach((canvasEl, idx) => {
    const title =
      canvasEl.querySelector(".gt-title")?.textContent?.trim() ||
      `Tijdlijn ${idx + 1}`;
...
function timelineSectionVertical(doc, title, nodes) {
  ...
  sec.innerHTML = `<h2 class="tl-title">${escapeHtml(
    title
  )}</h2><ol class="tl-list"></ol>`;
  ...
  li.innerHTML = `
    ...
    <div class="tl-card">
      <div class="tl-topline">
        ...
        <span class="tl-badge ${isIn ? "in" : "out"}">${
    isIn ? "IN" : "OUT"
  }</span>
      </div>
      <div class="tl-subline">
        <span class="tl-type">${
          n.type === "Voertuig" ? carIcon() : clockIcon()
        } ${escapeHtml(n.type)}</span>
...
function timelineSectionHorizontal(doc, title, nodes) {
  ...
  nodes.forEach((n, i) =>
    list.appendChild(makeHorizontalEntry(doc, n, i, nodes.length))
  );
  return sec;
}
